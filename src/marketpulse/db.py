...
def count_queries_today() -> int | None:
    """Count query_log rows logged since UTC midnight. None if DB is unavailable."""
    if not _db_available or _conn is None:
        return None
    sql = "SELECT count(*) FROM query_log WHERE queried_at >= date_trunc('day', now());"
    try:
        with _conn.cursor() as cur:
            cur.execute(sql)
            row = cur.fetchone()
    except Exception as exc:
        logger.warning("count_queries_today failed: %s", exc)
        return None
    return int(row[0]) if row else 0


def recent_queries(limit: int = 50) -> list[QueryLogRecord]:
    """Return the most recent query_log rows, newest first. Empty if DB unavailable."""
    if not _db_available or _conn is None:
        return []
    sql = """
        SELECT query, doc_grade, queried_at, coalesce(array_length(chunk_urls, 1), 0)
        FROM query_log
    ...
    try:
        ...
        logger.warning("recent_queries failed: %s", exc)
        return []
    return [
        QueryLogRecord(
            query=str(r[0]),

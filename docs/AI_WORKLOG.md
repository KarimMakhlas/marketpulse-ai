...
## 2026-05-30 — Wire dashboard screens to the backend + structure cleanup

**What changed:** Closed the gap between the design-system UI and the real API. Added four read endpoints — `GET /stats` (live Chroma chunk count + queries-today + active source count), `GET /sources` (the real 7–8 ingestion sources, credibility-weighted and sorted), `GET /queries` (recent `query_log` rows, auth-gated), and extended `GET /health` to return `model`/`default_k`/`db`/`redis`. Backed them with new pure-ish helpers: `collection_count()` (indexer), `count_queries_today()` + `recent_queries()` + a `QueryLogRecord` dataclass (db), and `list_sources()` + `SourceInfo` + a `SOURCE_DISPLAY` map (retriever). Rewired every previously-static screen: Dashboard KPIs, DataSources, Settings, and the sidebar footer now show live values; Query history fetches `/queries` with an honest sign-in/empty/error state; Monitoring is now an explicit "not wired" page instead of `Math.random()` charts. Deleted all fabricated data from the frontend — wrong model names (`claude-haiku-4-5`/`sonnet-4-5` → real `gemini-flash-latest` from `/health`), the 5 broken source-icon `<img>` refs (replaced with letter-badge fallbacks), fake nav badges (`2,481`/`9`/`2`), and the fake footer (`API 99.94%`/`Kafka lag 4 ms`/`Build v2.4.0`). Added a shared `EmptyState` primitive and an honest `—`/"Not wired" convention. Also flattened `docs/DECISIONS/` → `docs/ADR-0001…md` and `docs/screenshots/image1.png` → `docs/screenshot.png`. 11 new tests → 120 total; verified end-to-end in a real browser (Dashboard shows 283 indexed chunks, Sources shows 8 credibility-weighted feeds, no console errors).
**Files touched:** `src/marketpulse/api/app.py` + `schemas.py`, `src/marketpulse/db/client.py` + `__init__.py`, `src/marketpulse/ingestion/indexer.py`, `src/marketpulse/retrieval/retriever.py`, `frontend/src/api.js` + `Dashboard.jsx` + `DataSources.jsx` + `QueryHistory.jsx` + `Settings.jsx` + `Monitoring.jsx` + `components.jsx` + `App.jsx`, `tests/test_api.py` + `test_db.py` + `test_retriever.py`, `README.md`, `CLAUDE.md`, `docs/AI_WORKLOG.md`.
**Decisions made:** `/stats` and `/sources` are public (non-sensitive aggregate/config, and the screens render before the login flow); `/queries` requires auth since it exposes content. Honesty over fidelity (again): any metric the API can't back renders as `—` or an explicit "Not wired" panel rather than a fabricated number — this is the single rule applied across Dashboard, Monitoring, Query history, and Settings. Settings is read-only (no config-write API exists, so no fake "Save"); it shows live `/health` values plus the real compiled retrieval/embedding/auth constants. Did NOT build a periodic RAGAS/metrics endpoint — Monitoring documents what wiring it would take instead. `EmptyState` lives in `components.jsx` (loads first) to stay load-order-safe across the no-build Babel scripts.
